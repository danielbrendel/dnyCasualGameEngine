...
class CPlayerEntity : IScriptedEntity, IPlayerEntity
{
	...
	Vector m_vecSize;
	Model m_oModel;
	SpriteHandle m_hWalking;
	array<SpriteHandle> m_hShooting;
	float m_fRotation;
	uint32 m_uiButtons;
	uint32 m_uiHealth;
	...
	void OnSpawn(const Vector& in vec)
	{
		...
		this.m_hWalking = R_LoadSprite(g_szPackagePath + "gfx\\player\\move\\survivor-move_shotgun_0.png", 1, 64, 64, 1, true);
		for (int i = 0; i < 3; i++) {
			this.m_hShooting.insertLast(R_LoadSprite(g_szPackagePath + "gfx\\player\\shoot\\survivor-move_shotgun_" + i + ".png", 1, 64, 64, 1, true));
		}
		...
		this.m_oModel.Alloc();
		this.m_oModel.Initialize2(bbox, this.m_hWalking);
	}
	...
	void OnProcess()
	{
		...
		if ((this.m_uiButtons & BTN_ATTACK) == BTN_ATTACK) {
			...
			if (this.m_tmrAttack.IsElapsed()) {
				...
				Vector vecGunPos = Vector(this.m_vecPos[0] + 59 / 2, this.m_vecPos[1] + 52 / 2);
				gun.SetRotation(this.m_fRotation + 6.30 / 4);
				
				Ent_SpawnEntity("weapon_gun", @gun, vecGunPos);
				
				SoundHandle hSound = S_QuerySound(g_szPackagePath + "sound\\shoot.wav");
	...
	void OnDrawOnTop()
	{
		...
		if (!this.m_bAttacking) {
			R_DrawSprite(this.m_hWalking, Vector(Wnd_GetWindowCenterX() - 59 / 2, Wnd_GetWindowCenterY() - 52 / 2), 0, this.m_fRotation, Vector(-1, -1), 0.0f, 0.0f, bDrawCustomColor, sDrawingColor);
		} else {
			R_DrawSprite(this.m_hShooting[this.m_iAttackSprite], Vector(Wnd_GetWindowCenterX() - 59 / 2, Wnd_GetWindowCenterY() - 52 / 2), 0, this.m_fRotation, Vector(-1, -1), 0.0f, 0.0f, bDrawCustomColor, sDrawingColor);
			
			this.m_iAttackSprite++;
			if (this.m_iAttackSprite >= 3) {
				this.m_iAttackSprite = 0;
			}
		}
